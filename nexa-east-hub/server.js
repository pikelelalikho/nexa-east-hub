...
app.post('/login', loginLimiter, async (req, res) => {
  ...
  try {
      ...
      const token = jwt.sign({ email: user.email, role: user.role || 'user', id: user.id }, JWT_SECRET, { expiresIn: '24h' });
      ...
      try {
          ...
          res.json({ success: true, message: 'Login successful, but email notification failed', name: user.name, role: user.role || 'user', token });
      }
  } catch (error) {
...
app.delete('/admin/logs', authenticateAdminRoute, (req, res) => {
  ...
  try {
      ...
      saveLog({ eventType: 'logs_cleared', details: 'Admin cleared all logs', user: req.user.email });
